// pages/api/download-marksheets.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        // FIX: Removed unused 'exam_name' from destructuring
        const { results, class_name, section_name } = req.body;
        ...
        for (const student of (results as StudentResult[])) {
            const doc = new jsPDF();
            ...
            zip.file(`${student.roll_number}_${student.student_name}.pdf`, pdfOutput);
        }

        const zipContent = await zip.generateAsync({ type: "nodebuffer" });
        
        res.setHeader('Content-Type', 'application/zip');
        res.setHeader('Content-Disposition', `attachment; filename="marksheets_${class_name}_${section_name}.zip"`);
        res.send(zipContent);
